...
import bus_app.kafka.KafkaProducer;
import bus_app.kafka.KafkaMessageDto;
...
public class DriverService {
    ...
    public void processShift(Long id, String mode) {
        if (!mode.equals("ON") && !mode.equals("OFF")) {
            throw new IncorrectBodyException("Incorrect mode type!");
        }

        String phone = null;
        try {
            phone = driverRepository.findById(id).get().getPhone();
        } catch (NoSuchElementException ex) {
            throw new NoDataException("No driver with id: " + id + "!");
        }

        try {
            kafkaProducer.sendMessage(new KafkaMessageDto(
                    phone,
                    LocalDateTime.now().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME),
                    mode
            ));
        } catch (JsonProcessingException e) {
            throw new IncorrectBodyException(e.getMessage());
        }
    }
}